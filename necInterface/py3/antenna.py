...
class Antenna(individual.Individual):
    ...
    def randomInit(self):
        ...
        for i in range(numElements):
            self.addElement(element.MoxonElement())
    ...
    def mutate(self):
        ...
        if operation == 1:  # add
            if len(offs.elements) >= self.maxElements:
                ...
                offs.addElement(element.MoxonElement())
        if operation == 2:  # del
    ...
    def mate(self, partner):
        ...
        while len(offspring.elements) < nElements:
            #pick new element
            while True:
                e = random.choice(list(range(len(elementPool))))
    ...
    def checkDrivenElement(self):
        ...
        for e in self.elements:
            if type(e) == drivenElement.DrivenMoxonElement:
                if nDriven >= 1:
            ...
            n += 1
        if nDriven == 0:
            n = random.choice(list(range(len(self.elements))))
